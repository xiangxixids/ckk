...
\title{Multi-scale Quantum Harmonic Oscillator Algorithm for Multimodal Function Optimization
}
%
%
...
\begin{abstract}
%In this paper,Multi-scale Quantum Harmonic Oscillator Optimization Algorithm (MQHOA) is proposed to solve multimodal optimization problems.
...
\toprule
  The framework of  Multimodal MQHOA \\
\midrule
  begin\\
1:initialize{:}$k$, $m$, $\sigma_{min}$, $\sigma_s=d_{max}-d_{min}$, \\
2:randomly generate $x_i$($i$=1,2,3... $k$) in domain $[d_{min},d_{max}]$ \\
3:calculate the standard deviation $\sigma$ for $x_i$ \\
4:\textbf{do}\\
5:\qquad \textbf{do}\\
6:\qquad generate $x_{i\times j}$ $($j$=1,2,3... $m$)$ from $N(x_i,\sigma^2_s)$ \\
7:\qquad calculate f(x).\\
8:\qquad select $k$ optimal $x_i^{best}$ from \textbf{6}\\
9:\qquad update $x_i$:$x_i\gets x_i^{best}$\\
10:\qquad calculate the standard deviation $\sigma_k$\\
11:\qquad $\Delta$$\sigma$=$\|$$\sigma_k$-$\sigma$$\|$\\
11:\qquad \textbf{terminate rule}:$\Delta$$\sigma$<$\sigma_s$\\
12:\quad update $\sigma_s$:$\sigma_s\gets \frac{\sigma_s}{2}$\\
13:\textbf{terminate rule}($\sigma_s<\sigma_{min}$)\\
14:\textbf{output} $x'_{best},f_{best}$\\
  end\\
\bottomrule
\end{tabular}
...
\section{Evaluation}
In this section,the performance of MQHOA is studied.
\section{Conclusion}
